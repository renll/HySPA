...
class SequenceGenerator(nn.Module):
    ...
    def _generate(
        ...
        for step in range(max_len + 1):  # one extra step for EOS marker
            ...
            # print("step", step)
            # print("tokens[:, : step + 1]", tokens[:, : step + 1].shape)
            # print("tokens", tokens.shape)
            # print("encoder_outs", len(encoder_outs))
            # print("encoder_outs[0].shape", encoder_outs[0].shape)
            # print("incremental_states", len(incremental_states))
            ...
            # print("lprobs.shape[-1]",lprobs.shape[-1])
            #self.vocab_size=self.type_len + self.span_len * (src_len- self.type_len)#lprobs.shape[-1]
            # print("self.vocab_size=18 + 32 * src_len", 18 + 32 * src_len)
        ...
        #import pickle
        #pickle.dump(finalized,open("/home/liliang3/beam1.list","wb"))
        #raise ValueError()
        return finalized
...
class EnsembleModel(nn.Module):
    ...
    def forward_decoder(
        ...
        for i, model in enumerate(self.models):
            ...
            if decoder_len > 1 and decoder_out[1][2] is not None:
                if isinstance(decoder_out[1][2], Tensor):
                    attn = decoder_out[1][2]
                else:
                    attn_holder = decoder_out[1][2]["attn"]
            ...
            decoder_out_tuple = (
                ...
                None if decoder_len <= 1 else decoder_out[1][2],
            )
...
class SequenceGeneratorWithAlignment(SequenceGenerator):
    ...
    def _prepare_batch_for_alignment(self, sample, hypothesis):
        print("_prepare_batch_for_alignment")
        src_tokens = sample["net_input"]["src_tokens"]
        bsz = src_tokens.shape[0]
        src_tokens = (
            src_tokens[:, None, :]
