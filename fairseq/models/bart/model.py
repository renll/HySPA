...
from fairseq.models.HSDBtransformer import HSDBTransformerModel
...
@register_model("bart")
class BARTModel(HSDBTransformerModel):
    @classmethod
    def hub_models(cls):
        return {
        ...
        for k in state_dict.keys():
            ...
            if getattr(self.args, "load_checkpoint_heads", False):
                ...
                if head_name not in current_head_names:
                    ...
                    keys_to_delete.append(k)
        #keys_to_delete.append("decoder.embed_tokens.weight")
        #keys_to_delete.append("decoder.output_projection.weight")
        keys_to_delete.extend(k for k in state_dict.keys() if "decoder" in k)
...
def bart_large_architecture(args):
    ...
    args.decoder_ffn_embed_dim = getattr(
        args, "decoder_ffn_embed_dim", 2*768
    )
    ...
    args.decoder_forget_rate = getattr(args, "decoder_forget_rate", 0.0)
...
@register_model_architecture("bart", "bart_base_ie")
def bart_base_architecture(args):
    args.encoder_embed_dim = getattr(args, "encoder_embed_dim", 768)
    args.encoder_ffn_embed_dim = getattr(args, "encoder_ffn_embed_dim", 4 * 768)
    args.encoder_layers = getattr(args, "encoder_layers", 6)
