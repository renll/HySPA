...
class RobertaEmbeddings(nn.Module):
    ...
    def __init__(self, config):
        ...
        #self.entity_id_embeddings = nn.Embedding(config.entity_id_size, config.hidden_size, padding_idx=0)
        #self.entity_type_embeddings = nn.Embedding(config.entity_type_size, config.hidden_size, padding_idx=0)
        ...
        #self.dropout = FairseqDropout(config.input_dropout_prob)
    ...
    def forward(self, input_ids=None, token_type_ids=None, position_ids=None, inputs_embeds=None,entity_type_ids=None):
        ...
        if inputs_embeds is None:
            ...
            inputs_embeds = utils.embedded_dropout(
                            self.word_embeddings, input_ids.transpose(0,1),
                            dropout=self.dropoute if self.training else 0)
            inputs_embeds = inputs_embeds.transpose(0,1)
        position_embeddings = self.position_embeddings(position_ids)
        #token_type_embeddings = self.entity_id_embeddings(token_type_ids)
...
class RobertaSelfOutput(nn.Module):
    def __init__(self, config):
        ...
        self.LayerNorm = nn.LayerNorm(config.hidden_size, eps=config.layer_norm_eps)
        self.dropout = FairseqDropout(config.hidden_dropout_prob)

    def forward(self, hidden_states, input_tensor):
        hidden_states = self.dense(hidden_states)
        hidden_states = hidden_states.transpose(0,1)
        hidden_states = self.dropout(hidden_states, locked=False)
        hidden_states = hidden_states.transpose(0,1)
...
class RobertaPreTrainedModel(PreTrainedModel):
    ...
    def _init_weights(self, module):
        ...
        if isinstance(module, (nn.Linear, nn.Embedding)):
            ...
            utils.truncated_normal_(module.weight)
...
class RobertaModel(RobertaPreTrainedModel):
    ...
    def forward(
        ...
        embedding_output = self.embeddings(
            input_ids=input_ids, position_ids=position_ids, token_type_ids=token_type_ids, inputs_embeds=inputs_embeds,entity_type_ids=entity_type_ids,
        )
